...
const Select: NextPage<SelectProps> = ({ options, initialLabel }) => {
    ...
    const renderOptions = options.map(x => {
        return <div key={x.value} data-value={x.value} className="p-4 text-main bg-white hover:bg-gray-200" onClick={() => setSelected(x)}>{x.label}</div>
    })
    ...
    return (
        <div className={`py-3 px-3 border-solid border-main font-poppins rounded-normal h-14 cursor-pointer relative ${!dropDownOpen && !selected ? "border" : "border-2"}`} onClick={toggleDropDown} ref={mainContainer}>
            ...
            <div className="font-medium text-xl text-main">
                <span className="text-gray-500 translate-x-0 translate-y-0 scale-100 duration-300 transition-all absolute bg-white" ref={labelRef} >{initialLabel.label}</span>
                {selected && <span data-selected={selected.value} className="text-xl text-main">{selected.label}</span>}
            </div>
