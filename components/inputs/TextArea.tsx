...
const TextArea: NextPage<InputProps> = ({ type, label }) => {
    ...
    return (
        <div className="relative w-aute px-3 h-56">
            <textarea className="absolute focus:outline-none top-0 left-0 text-xl w-full h-full py-3 px-3 border border-solid border-gray-600 font-poppins text-main rounded-normal" onFocus={handleFocus} onBlur={handleBlur} ref={inputRef}/>
            <span className="absolute bg-white origin-left text-gray-500 text-xl transition-all translate-x-0 duration-300 scale-100 translate-y-0 px-1 top-3" onClick={() => inputRef.current?.focus()}>{label}</span>
        </div>
    )
}
