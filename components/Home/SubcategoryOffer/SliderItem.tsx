...
const SliderItem: NextPage<Props> = ({ subcategoryOfferItem }) => {
    ...
    const renderImages = subcategoryOfferItem.subcategory_images.map((x, i) => {
        return (
            <div key={i} className="relative w-28 h-28 sm:w-32 sm:h-32 desktop:w-40 desktop:h-40 desktop-big:w-32 desktop-big:h-32 desktop-large:w-36 desktop-large:h-36 bg-white rounded-lg p-4">
